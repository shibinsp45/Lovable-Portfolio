...
const Header = () => {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-background/60 backdrop-blur-xl border-b border-border/30" : ""}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 sm:py-4 py-[37px]">
        <nav className="flex items-center justify-center relative">
          {/* Logo - Left (absolute) */}
          <Link to="/" className="absolute left-0 flex items-center gap-3 group">
            <img src={isDark ? logoDark : logoLight} alt="Logo" className="h-12 md:h-10 w-auto object-contain transition-all duration-300 group-hover:scale-110 invert dark:invert-0" />
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1 px-2 py-2 rounded-full border border-border/40 bg-card/30 backdrop-blur-xl">
            {navLinks.map(link => <a key={link.name} href={link.href} className="font-sans text-sm px-5 py-2 text-muted-foreground hover:text-foreground hover:bg-secondary/50 rounded-full transition-all duration-200">
                {link.name}
              </a>)}
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 font-sans text-sm px-5 py-2 text-foreground font-medium hover:text-primary hover:bg-secondary/50 rounded-full transition-all duration-200">
              LinkedIn
            </a>
          </div>
          ...
          <div className="absolute right-0 hidden md:flex">
            <Button variant="ghost" size="icon" onClick={toggleTheme} className="h-10 w-10 rounded-full border border-border/40 bg-card/30 backdrop-blur-xl hover:bg-secondary/50">
              {isDark ? <Sparkles className="h-4 w-4 text-foreground" /> : <Sun className="h-4 w-4 text-foreground" />}
            </Button>
          </div>

          {/* Mobile Menu Button */}
          <div className="absolute right-0 flex md:hidden items-center gap-3">
            <Button variant="ghost" size="icon" onClick={toggleTheme} className="h-11 w-11 rounded-full border border-border/40 bg-card/30 backdrop-blur-xl">
              {isDark ? <Sparkles className="h-5 w-5 text-foreground" /> : <Sun className="h-5 w-5 text-primary" />}
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="h-11 w-11 rounded-full border border-border/40 bg-card/30 backdrop-blur-xl">
